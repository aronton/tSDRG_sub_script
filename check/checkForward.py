...
Ls = [16,32,48,64,80,96,128,160,192,256,384,512]
# Jdis = ["Jdis050","Jdis100","Jdis150","Jdis180","Jdis200"]

final_seed = np.zeros([len(Ls),len(Jdis),len(Dim)],dtype=int)
seedRange = np.linspace(0, checkRangeFinalSeed, 101, dtype=int)
seedRange[0] = 1
...
np.save("seedArray" + str(spin) + "range_" + str(1) + "_" + str(checkRangeFinalSeed) + str(BC) + ".npy", final_seed)
seedArrayFrame = []

for k in range(len(Ls)):
    seedArrayFrame.append( pd.DataFrame( final_seed[k,:,:], columns = Dim, index = Jdis, dtype = int))

seedArrayxlsx = pd.ExcelWriter("seedArray" + str(spin) + "range_" + str(1) + "_" + str(checkRangeFinalSeed) + str(BC) + ".xlsx", engine='xlsxwriter')
